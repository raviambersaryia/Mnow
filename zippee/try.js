...
<table class="table table-bordered"><thead><tr>
        <th>Store Name</th>
        <th>Projected Orders || 5/9/2025</th>
        <th>Projected Orders | till 4-5 PM</th>
        <th>Projected Orders | till --- (15% Buffer)</th>
        <th>Total Orders</th>
        <th>Cancelled %</th>
        <th>Delivered Orders</th>
        <th>Deep Pain %</th>
        ...
        <th>Idle rider</th>
        <th>BF</th>
        <th>Order Attainment (Projected Orders | till 4-5 PM)</th>
    ...
    allStores.forEach(store => {
        ...
        // Projection columns
        mergedHTML += `<td>${projectionData[store] ? projectionData[store][0] : ''}</td>`; // Projected Orders || 5/9/2025
        mergedHTML += `<td>${projectionData[store] ? projectionData[store][1] : ''}</td>`; // Projected Orders | till 4-5 PM
        mergedHTML += `<td>${projectionData[store] ? projectionData[store][2] : ''}</td>`; // Projected Orders | till --- (15% Buffer)

        // Summary columns
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][0] : ''}</td>`; // Total Orders
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][3] : ''}</td>`; // Cancelled %
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][4] : ''}</td>`; // Delivered Orders
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][7] : ''}</td>`; // Deep Pain %
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][8] : ''}</td>`; // Actual Riders
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][9] : ''}</td>`; // Idle rider
        mergedHTML += `<td>${summaryData[store] ? summaryData[store][10] : ''}</td>`; // BF

        // Order Attainment (Projected Orders | till 4-5 PM)
        let projectedOrders = projectionData[store] ? parseFloat(projectionData[store][1]) || 0 : 0;
        let totalOrders = summaryData[store] ? parseFloat(summaryData[store][0]) || 0 : 0;
        let orderAttainment = projectedOrders ? ((totalOrders / projectedOrders) * 100).toFixed(2) + '%' : '0.00%';
        mergedHTML += `<td>${orderAttainment}</td>`;

        // Deep Pain (Order Count)
        let deepPainPercent = summaryData[store] ? parseFloat(summaryData[store][7]) || 0 : 0;
        let deepPainOrderCount = (totalOrders * (deepPainPercent / 100)).toFixed(2);
        deepPainOrderCounts.push(deepPainOrderCount);
        mergedHTML += `<td>${deepPainOrderCount}</td>`;
    ...
    mergedHTML += `<tfoot><tr style="background:#000;color:#fff;font-weight:bold;">
        <td>Total</td>
        <td>${projectionTotals[0]}</td>
        <td>${projectionTotals[1]}</td>
        <td>${projectionTotals[2]}</td>
        <td>${summaryTotals[0]}</td>
        <td>${summaryTotals[3]}</td>
        <td>${summaryTotals[4]}</td>
        <td>${summaryTotals[7]}</td>
        <td>${summaryTotals[8]}</td>
        <td>${summaryTotals[9]}</td>
        <td>${summaryTotals[10]}</td>
        <td></td>
        <td>${deepPainOrderCounts.reduce((a, b) => parseFloat(a) + parseFloat(b), 0).toFixed(2)}</td>
        <td>${additionalOrdersList.reduce((a, b) => a + b, 0)}</td>
        <td></td>
        <td></td>
    </tr></tfoot>`;

    // 7. Add "Mnow Data" section
    mergedHTML += `</tbody></table>
    <div style="margin-top: 20px;">
        <h3>Mnow Data</h3>
        <p>Here is some additional information about Mnnow stores:</p>
        <ul>
            <li>Total Mnnow Stores: ${allStores.filter(store => store.toLowerCase().includes('mnow')).length}</li>
            <li>Average Deep Pain Order Count: ${deepPainOrderCounts.reduce((a, b) => parseFloat(a) + parseFloat(b), 0) / allStores.length}</li>
            <!-- Add more Mnnow data here -->
        </ul>
    </div>`;

    // 8. Add 3rd animation section
    mergedHTML += `<div id="animation-container" style="margin-top: 20px;">
        <h3>Animation Section</h3>
        <p>This is where the 3rd animation will go.</p>
